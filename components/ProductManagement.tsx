...
import { Product, Outlet, ProductCategory, Variant, RentalResource, ResourceAvailability } from '../types';
...
const ProductModal: React.FC<{
  ...
  const [formData, setFormData] = useState<ProductFormData>({
    ...
    type: 'barang',
    outletId: '',
    price: { general: 0, member: 0 }
  });
  const [variants, setVariants] = useState<VariantFormData[]>([{ name: '', sku: '', price: { general: 0, member: 0 }, stock: 0 }]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const baseData: ProductFormData = {
        ...
        price: product?.price || { general: 0, member: 0 },
      };
      ...
      if (product) {
        if (product.type === 'barang') {
          ...
          setVariants(productVariants.length > 0 ? productVariants : [{ name: '', sku: '', price: { general: 0, member: 0 }, stock: 0 }]);
        ...
        setVariants([{ name: '', sku: '', price: { general: 0, member: 0 }, stock: 0 }]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if(finalProductData.type === 'barang') delete finalProductData.price;
  ...
  const handleVariantChange = (index: number, field: keyof VariantFormData, value: any) => {
    ...
    if (field === 'price') {
      newVariants[index].price = { ...newVariants[index].price, ...value };
    } else {
      (newVariants[index] as any)[field] = value;
    }
    setVariants(newVariants);
  };
  const addVariant = () => setVariants([...variants, { name: '', sku: '', price: { general: 0, member: 0 }, stock: 0 }]);
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 md:p-8 w-full max-w-2xl max-h-full overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" value={formData.name} onChange={e => setFormData(p => ({...p, name: e.target.value}))} placeholder="Nama Produk" required className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"/>
            <select name="categoryId" value={formData.categoryId} onChange={e => setFormData(p => ({...p, categoryId: e.target.value}))} required disabled={relevantCategories.length === 0} className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-slate-100">
          ...
          <textarea value={formData.description} onChange={e => setFormData(p => ({...p, description: e.target.value}))} placeholder="Deskripsi Produk" required className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" rows={2}/>
          {/* 
            <select name="outletId" value={formData.outletId} required disabled className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 bg-slate-100">
            {allData.outlets.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
          </select>
           */}
          
          {/* Type Selector */}
          <div className="flex rounded-md shadow-sm">
              <button type="button" onClick={() => setFormData(p=>({...p, type: 'barang'}))} disabled={!!product} className={`px-4 py-2 border rounded-l-md w-1/2 ${formData.type === 'barang' ? 'bg-indigo-600 text-white' : 'bg-white hover:bg-slate-50'} disabled:opacity-50`}>Produk Barang</button>
              <button type="button" onClick={() => setFormData(p=>({...p, type: 'sewa'}))} disabled={!!product} className={`px-4 py-2 border rounded-r-md w-1/2 ${formData.type === 'sewa' ? 'bg-indigo-600 text-white' : 'bg-white hover:bg-slate-50'} disabled:opacity-50`}>Produk Sewa</button>
          </div>
                  ...
                  {variants.map((variant, index) => (
                      <div key={index} className="p-3 border rounded bg-slate-50 space-y-2">
                          <div className="flex items-center gap-2">
                              <input type="text" placeholder="Nama Varian (e.g. Merah, XL)" value={variant.name} onChange={e => handleVariantChange(index, 'name', e.target.value)} required className="w-full px-2 py-1 border border-slate-300 rounded-md text-sm"/>
                              <input type="text" placeholder="SKU" value={variant.sku} onChange={e => handleVariantChange(index, 'sku', e.target.value)} required className="w-1/2 px-2 py-1 border border-slate-300 rounded-md text-sm"/>
                              <button type="button" onClick={() => removeVariant(index)} className="text-red-500 hover:text-red-700 disabled:opacity-50" disabled={variants.length <= 1}><TrashIcon className="w-5 h-5"/></button>
                          </div>
                          <div className="grid grid-cols-3 gap-2">
                              <input type="number" placeholder="Harga Umum" value={variant.price.general} onChange={e => handleVariantChange(index, 'price', { general: Number(e.target.value) })} required className="w-full px-2 py-1 border border-slate-300 rounded-md text-sm"/>
                              <input type="number" placeholder="Harga Member" value={variant.price.member} onChange={e => handleVariantChange(index, 'price', { member: Number(e.target.value) })} required className="w-full px-2 py-1 border border-slate-300 rounded-md text-sm"/>
                              <input type="number" placeholder="Stok" value={variant.stock} onChange={e => handleVariantChange(index, 'stock', Number(e.target.value))} required className="w-full px-2 py-1 border border-slate-300 rounded-md text-sm"/>
                          </div>
                      </div>
                  ))}
                </div>
                <button type="button" onClick={addVariant} className="text-sm text-indigo-600 hover:text-indigo-800 font-semibold flex items-center gap-1"><PlusIcon className="w-4 h-4"/> Tambah Varian</button>
            </div>
          ) : (
            <div className="space-y-4 p-4 border rounded-md">
                <h3 className="font-semibold text-slate-700">Harga & Sumber Daya Sewa</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input type="number" value={formData.price?.general} onChange={e => setFormData(p => ({...p, price: {...p.price!, general: Number(e.target.value)}}))} placeholder="Harga Umum / jam" required className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"/>
                  <input type="number" value={formData.price?.member} onChange={e => setFormData(p => ({...p, price: {...p.price!, member: Number(e.target.value)}}))} placeholder="Harga Member / jam" required className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"/>
                </div>
                <div className="space-y-3">
                      ...
                      <div key={resIndex} className="p-3 border rounded bg-slate-50 space-y-2">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-1 mb-2">
                                {weekDays.map(day => (
                                    <button type="button" key={day} onClick={() => handleAvailabilityToggle(resIndex, day)} className={`px-2 py-0.5 text-xs border rounded-full ${res.availabilities.some(a => a.dayOfWeek === day) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white hover:bg-slate-100'}`}>{day}</button>
                                ))}
                            </div>
                ...
                <button type="button" onClick={addResource} className="text-sm text-indigo-600 hover:text-indigo-800 font-semibold flex items-center gap-1"><PlusIcon className="w-4 h-4"/> Tambah Resource</button>
            </div>
          )}
...
const ProductManagement: React.FC = () => {
    ...
    const { businessUnits, products, outlets, categories, variants, rentalResources, resourceAvailabilities, addProduct, updateProduct, deleteProduct } = data;
    ...
    const getPriceRangeString = (product: Product) => {
        if (product.type === 'sewa') {
            return `Rp${product.price?.general.toLocaleString('id-ID')}`;
        }
        const productVariants = variants.filter(v => v.productId === product.id);
        if (productVariants.length === 0) return 'N/A';
        const prices = productVariants.map(v => v.price.general);
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                    <select value={selectedUnit} onChange={(e) => setSelectedUnit(e.target.value)} className="w-full sm:w-auto px-4 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                      {businessUnits.map(unit => <option key={unit.id} value={unit.id}>{unit.name}</option>)}
                    </select>
                    <select value={selectedOutlet} onChange={(e) => setSelectedOutlet(e.target.value)} className="w-full sm:w-auto px-4 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" disabled={availableOutlets.length === 0}>
            ...
            <div className="flex justify-end">
                <button onClick={() => handleOpenModal()} className={`flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition ${!selectedOutlet ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!selectedOutlet}>
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {filteredProducts.map((product) => {
                                ...
                                <tr key={product.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{getPriceRangeString(product)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button onClick={() => handleOpenModal(product)} className="text-indigo-600 hover:text-indigo-900 mr-4"><EditIcon className="w-5 h-5"/></button>
            ...
            {isModalOpen && <ProductModal 
                ...
                allData={{ outlets, categories, variants, rentalResources, resourceAvailabilities }}
