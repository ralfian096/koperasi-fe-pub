...
const PromotionModal: React.FC<PromotionModalProps> = ({ isOpen, onClose, onSave, promoToEdit, selectedBusinessUnit }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 p-4">
                ...
                <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-2 space-y-6">
                    ...
                    <div className="p-4 border rounded-lg space-y-3">
                        ...
                        <div className="flex border-b">
                            <button type="button" onClick={() => handleMainRewardTypeChange('DISCOUNT')} className={`px-4 py-2 text-sm font-semibold ${mainRewardType === 'DISCOUNT' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500'}`}>Potongan Harga</button>
                            <button type="button" onClick={() => handleMainRewardTypeChange('FREE_ITEM')} className={`px-4 py-2 text-sm font-semibold ${mainRewardType === 'FREE_ITEM' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500'}`}>Gratis Produk</button>
                        </div>
                        <div className="pt-2">
            ...
            <style>{`.input { display: block; width: 100%; padding: 0.5rem 0.75rem; border: 1px solid #cbd5e1; border-radius: 0.375rem; } .btn-primary { padding: 0.5rem 1rem; background-color: #4f46e5; color: white; border-radius: 0.5rem; font-weight: 600; } .btn-primary:disabled { background-color: #a5b4fc; cursor: not-allowed;} .btn-secondary { padding: 0.5rem 1rem; background-color: #e2e8f0; color: #1e293b; border-radius: 0.5rem; font-weight: 600; }`}</style>
        </div>
    );
};
